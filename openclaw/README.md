...
A collection of skills adapted for the OpenClaw platform, containing 21 skill modules. Includes all HogAgent core investment research skills plus 11 additional ones (`deliver_files`, `hog-memory`, and 9 extra utility tools).
...
| `deliver_files` | 2.1.0 | Compatibility wrapper for restricted Gateway file delivery, now also available in `hog-gateway-tools` |
| `hog-gateway-tools` | 3.5.0 | Authenticated General MCP CLI: task result reporting, restricted file delivery, Work context, Task Resource status, Knowledge, Memory, notifications, watchlist and resource recommendations |
...
| `hog-kb-tools` | 1.2.0 | Gateway KB MCP Server CLI: knowledge base search and cross-session memory management |
...
| `hog-memory` | 1.3.0 | Cross-session persistent memory CLI: save, search and recall market insights and research conclusions |
...
For `hedgehog-company-index-data`, `hedgehog-macro-industry-data`, and `hedgehog-news-reports`, configure `skills.entries.<skill>.apiKey` in `~/.openclaw/openclaw.json`. Each skill declares `metadata.openclaw.primaryEnv: CIWEIAI_API_KEY`, so OpenClaw injects the configured key for the host Agent run. Existing `CIWEIAI_API_KEY` values are preserved by OpenClaw; `API_KEY` and HogAgent config remain compatibility fallbacks.
