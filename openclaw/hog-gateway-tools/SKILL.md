...
```bash
node <skill_path>/cli.mjs <command> [arguments]
```
...
```bash
node <skill_path>/cli.mjs report-task-result task-123 \
  --content "Task output" --summary "Summary" \
  --delivery-files-json '[{"name":"report.pdf","path":"tasks/task-123/report.pdf","summary":"Report"}]'

node <skill_path>/cli.mjs deliver-files tasks/task-123/report.pdf tasks/task-123/chart.png \
  --summary "Analysis artifacts" --task-id task-123
node <skill_path>/cli.mjs deliver-files --files-json \
...
node <skill_path>/cli.mjs send-notification workflow_complete "Analysis complete" "The report is ready"

node <skill_path>/cli.mjs get-watchlist

node <skill_path>/cli.mjs recommend-resource --source-type skill --title "Weekly report" \
  --content-type report --summary "Sector review" --recommend-reason "Relevant to the watchlist"

node <skill_path>/cli.mjs kb-search "白酒行业景气度" --type Research --limit 5
node <skill_path>/cli.mjs kb-get 3f1c2b9a-uuid

node <skill_path>/cli.mjs memory-search "茅台" --stock-codes "600519.SH" --limit 10
node <skill_path>/cli.mjs memory-save "茅台双底形态已确认" \
  --task-type market_insight --tags "600519.SH,食品饮料,双底" --work-id work-123
node <skill_path>/cli.mjs memory-recall memory-123
node <skill_path>/cli.mjs memory-update memory-123 --content "目标价调整为 1900"
```
