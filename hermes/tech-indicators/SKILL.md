...
### calc.mjs — 指标计算主脚本
```bash
node ${HERMES_SKILL_DIR}/scripts/calc.mjs <data.json> <output> [--indicators sma,ema,rsi,...] [--params-file "<workspace>/tmp-tech-indicators-<id>.json"] [--format json|markdown]
```
...
# 自定义参数
node ${HERMES_SKILL_DIR}/scripts/calc.mjs ohlcv.json result.json --indicators sma,rsi --params-file "<workspace>/tmp-tech-indicators-<id>.json"
