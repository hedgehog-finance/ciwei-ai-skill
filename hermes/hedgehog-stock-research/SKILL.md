...
3. 暂存基本面关键结论（评级方向、估值区间、核心风险，各≤50字），供 Stage 3 使用。
...
2. 立即 `write(append:true)` 写入 `## 四、 技术面量化（择时依据）` + 报告全文内容（原样搬入，禁止压缩，剥离尾注暂时存供 Stage 4 使用）。
...
【报告摘要】：[核心论点和博弈焦点、市场预期与交易主线、跟踪焦点与催化剂]
【策略思路】：[非常简短的指导性策略思路（不得有买卖点指导）]
